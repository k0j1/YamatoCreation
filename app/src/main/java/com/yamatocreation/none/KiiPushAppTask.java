...
import com.yamatocreation.none.ProgressDialogFragment;
...
/*
	private String doUnsubscribeBucket() {
		try {
			...
			KiiBucket bucket = user.bucket(SyncStateContract.Constants.PUSH_BUCKET_NAME);
	...
	private String doCreateUscopeTopic() {
		try {
			assertPushRegistred();
			KiiTopic topic = KiiUser.topic(SyncStateContract.Constants.USERTOPIC);
			topic.save();
		} catch (Exception e) {
			this.e = e;
		}
